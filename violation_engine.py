...
class ViolationEngine:
    def __init__(self, model_path, plate_model_path=None, conf=0.25):
        # Load YOLO model
        ...
        if not groq_api_key:
            raise ValueError(
                "GROQ_API_KEY not found. Please set it in a .env file or environment variables."
            )
    ...
    def get_plate_text_groq(self, crop_np):
        """Send cropped plate image to Groq Vision model"""
                        ...
                        "content": [
                            {
                                "type": "text",
                                "text": (
                                    "Read the license plate text from this image. "
                                    "Output ONLY the alphanumeric characters."
                                ),
                            },
                            {
                                "type": "image_url",
    ...
    def detect_violations_full_frame(self, frame, frame_count, is_video=True):
        ...
        if results.boxes is not None:
            ...
            for box, cls, track_id in zip(boxes, clss, ids):
                ...
                if any(k in label for k in ["motorcycle", "bike", "scooter"]):
                ...
                elif any(k in label for k in ["no helmet", "no_helmet", "missing helmet"]):
                    violations.append(
                        {"box": [x1, y1, x2, y2], "type": "No Helmet"}
                    )
                    cv2.rectangle(annotated, (x1, y1), (x2, y2), (0, 0, 255), 2)

                elif "mobile" in label or "phone" in label:
                    violations.append(
                        {"box": [x1, y1, x2, y2], "type": "Mobile Usage"}
                    )
        ...
        for bike in bikes:
            ...
            should_process = (
                not is_video
                or (bike_id != -1 and bike_id not in self.processed_violation_ids)
            )
            ...
            if should_process:
                ...
                evidence_img = (
                    best_plate_crop
                    if best_plate_crop is not None
                    else frame[by1:by2, bx1:bx2]
                )

                count_val = (
                    len(self.processed_violation_ids)
                    if is_video
                    else np.random.randint(0, 1000)
                )

                cv2.imwrite(
                    f"saved_violations/viol_{frame_count}_{count_val}.jpg",
                    evidence_img,
                )
